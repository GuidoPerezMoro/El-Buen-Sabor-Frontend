...
const ModalPromocion: React.FC<ModalPromocionProps> = ({
  ...
  const [selectedArticles, setSelectedArticles] = useState<
    { idArticulo: number; cantidad: number; denominacion: string }[]
  >([]);
  ...
  useEffect(() => {
    const articulosGuardados = JSON.parse(
      localStorage.getItem("articulosSeleccionados") || "[]"
    );
  ...
  const handleSubmit = async (values: PromocionPost) => {
    ...
    try {
      ...
      if (isEditMode && promocionAEditar) {
        response = await promocionService.put(
          `${URL}/promocion`,
          promocionAEditar.id,
          promocionPost
        );
        ...
        response = (await promocionService.post(
          `${URL}/promocion`,
          promocionPost
        )) as IPromocion;
        id = response.id;
            ...
            <List>
              {selectedArticles.map((article, index) => (
                <ListItem key={article.idArticulo}>
                  <ListItemText
                    primary={`${article.denominacion} - ${article.cantidad}`}
                  />
                  <ListItemSecondaryAction>
                    <IconButton
                      edge="end"
